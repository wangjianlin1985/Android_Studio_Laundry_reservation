...
public class UserInfoDAO {
    ...
    public ArrayList<UserInfo> QueryUserInfoInfo(String user_name,String name,String birthDate,String regTime,int currentPage) { 
    	Session s = factory.getCurrentSession();
    	...
    	if(!regTime.equals("")) hql = hql + " and userInfo.regTime like '%" + regTime + "%'";
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<UserInfo> QueryUserInfoInfo(String user_name,String name,String birthDate,String regTime) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From UserInfo userInfo where 1=1";
    	if(!user_name.equals("")) hql = hql + " and userInfo.user_name like '%" + user_name + "%'";
    	if(!name.equals("")) hql = hql + " and userInfo.name like '%" + name + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String user_name,String name,String birthDate,String regTime) {
        Session s = factory.getCurrentSession();
        String hql = "From UserInfo userInfo where 1=1";
        if(!user_name.equals("")) hql = hql + " and userInfo.user_name like '%" + user_name + "%'";
        if(!name.equals("")) hql = hql + " and userInfo.name like '%" + name + "%'";
