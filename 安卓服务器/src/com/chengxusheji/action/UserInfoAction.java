...
public class UserInfoAction extends BaseAction {
    ...
    public String QueryUserInfo() {
        ...
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name, name, birthDate, regTime, currentPage);
        /*计算总的页数和总的记录数*/
        userInfoDAO.CalculateTotalPageAndRecordNumber(user_name, name, birthDate, regTime);
        /*获取到总的页码数目*/
    ...
    public String QueryUserInfoOutputToExcel() { 
        ...
        if(birthDate == null) birthDate = "";
        if(regTime == null) regTime = "";
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name,name,birthDate,regTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","生日","个人照片","联系电话","纬度","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),new SimpleDateFormat("yyyy-MM-dd").format(userInfo.getBirthDate()),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getLatitude() + "",userInfo.getRegTime()});
        }
        /*
